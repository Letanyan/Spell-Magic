	...
	while index < area.size() - 1:
		...
		match struct:
			...
			TUNDRA_STRUCTURES_KIND.FLAT_ROCK:
				...
				for i in count:
					var p := pop.spawn_foliage(World.Foliage.ROCK_FLATTOP, state, pos + Rand.point_in_circle_2d(10, rng), spacing)
					if p != null: result.append(p)
				
			TUNDRA_STRUCTURES_KIND.LONE_HEAD:
				var pos := area[index]
				var p := pop.spawn_enemy(World.Enemy.WALKER_HEAD, state, pos, spacing)
				if p != null: result.append(p)
				
			TUNDRA_STRUCTURES_KIND.LONE_WALKER:
				var pos := area[index]
				var p := pop.spawn_enemy(World.Enemy.WALKER, state, pos, spacing)
				if p != null: result.append(p)
			...
			TUNDRA_STRUCTURES_KIND.RABBIT:
				...
				for i in count:
					var p := pop.spawn_enemy(World.Enemy.WALKER_HEAD, state, pos + Rand.point_in_circle_2d(10, rng), spacing)
					if p != null: result.append(p)
