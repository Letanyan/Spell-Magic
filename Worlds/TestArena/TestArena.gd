...
func setup(_settings: WorldSettings) -> void:
	...
	#add_enemy(hot_blob)
	#var bluemon := Population.generate_enemy(World.Enemy.BLUEMON, player, -20, 1000, -20)
	#add_enemy(bluemon)
	#var frog := Population.generate_enemy(World.Enemy.FROG, player, 20, 1000, -20)
	#add_enemy(frog)
	#var mushking := Population.generate_enemy(World.Enemy.MUSHKING, player, -20, 1000, 20)
	#add_enemy(mushking)
	#var rabbit := Population.generate_enemy(World.Enemy.RABBIT, player, 20, 1000, 20)
	#add_enemy(rabbit)
	#var batty := Population.generate_enemy(World.Enemy.BATTY, player, -20, 1000, -20)
	#add_enemy(batty) 
	#var bee := Population.generate_enemy(World.Enemy.BEE, player, 20, 1000, -20)
	#add_enemy(bee) 
	#var bumble_bee := Population.generate_enemy(World.Enemy.BUMBLE_BEE, player, -20, 1000, 20)
	#add_enemy(bumble_bee) 
	#var undead_head := Population.generate_enemy(World.Enemy.UNDEAD_HEAD, player, 20, 1000, 20)
