...
enum Tutorials { CONTROLS, SPELLS, ARTIFACTS, COINS, NOTES, WANDS, KEYS, GOT_UPGRADE, PLAYER_MESSAGE }
...
**{tag}: {tag} controls when entities are shown and how to defeat a level. 
	- Entities with the highest tag value are shown:
		- Only once all enemies with the highest tag values are defeated will all enemies with the next highest tag value be shown.
		- Only once all flags with the highest tag value are collected will all flags with the next highest tag value be shown.
	- Entities with tag 0 are a victory condition: 
		- When the player defeats a enemy with tag 0 the level will be defeated.
