...
static var notes := {
	"func rot_x, rot_y, rot_z": "{rot_#(a, vx, vy, vz, px, py, pz)} can be used to rotate a point around a vector and returns the component {#}. The parameters of the functions are defined as {a} the angle around the vector {(vx, vy, vz)} which the point {(px, py, pz)} is rotated around.",
	...
	"func dot2, dot3": "{dot2(vx,vy)} and {dot3(vx,vy,vz)} returns the dot product of the parameters. The exact definition of the functions are {vx*vx+vy*vy} and {vx*vx+vy*vy+vz*vz} respectively.",
	"func cross_x, cross_y, cross_z": "{cross_#(vx,vy,vz,wx,wy,wz)} returns the component {#} of the cross product of 2 vector (vx,vy,vz) and (wx,wy,wz). The exact defintion for each function {cross_x}, {cross_y} and {cross_z} is {(vx*wy - vy*wx)}, {(vz*wx - vx*wz)} and {(vy*wz - vz*wy)} respectively",
	"func proj_x, proj_y, proj_z": "{proj_#(vx,vy,vz,wx,wy,wz)} returns the component {#} of projected vector of (vx,vy,vz) on to the plane defined by normal vector (wx,wy,wz).",
	...
	"func lte": "{lt(x,y)} returns {1} if {x} is less than or equal to {y} else {0} is returned.",
	"func gt": "{lt(x,y)} returns {1} if {x} is greater than {y} else {0} is returned.",
	"func gte": "{lt(x,y)} returns {1} if {x} is greater than or equal {y} else {0} is returned.",
	...
	"spell x, y, z": "The 3d coordinates defined by an expression for each projectile of the spell in the world. These values are updated each time step.",
	...
	"variable ru, rv, rw": "{ru} describes the angle from the positive {x} axis to the {x} axis of the players aim. Similarily {rv} describes the {z} axis angle. While {rw} describes the angle to the {y} axis.",
	"variable rU, rV, rW": "{rU} describes the angle from the positive {x} axis to the {x} axis of an enemy that was in line with the player aim when the spell was cast. Similarily {rV} and {rW} describes the angle for the {z} and {y} axis.",
	"variable i, j, k": "A unit vector {(i,j,k)} describing the direction the player is moving.",
	"variable I, J, K": "A unit vector {(I,J,K)} describing the direction from the player to the projectile.",
	"variable ri, rj, rk": "{ri} describes the angle from the positive {x} axis to the {x} axis of the players direction of movement. Similarily {rj} describes the {z} axis angle. While {rk} describes the angle to the {y} axis.",
	"variable rI, rJ, rK": "{rI} describes the angle from the positive {x} axis to the {x} axis of each spell projectile. Similarily {rJ} describes the {z} axis angle. While {rK} describes the angle to the {y} axis.",
	"variable tu, tv, tw, tU, tV, tW, ti, tj, tk, tI, tJ, tK, tru, trv, trw, trU, trV, trW, tri, trj, trk, trI, trJ, trK": "Each of these values are the same as their values if you strip the 't' prefix. However these values will be updated at each time step {t}",
	"variable Tu, Tv, Tw, TU, TV, TW, Ti, Tj, Tk, TI, TJ, TK, Tru, Trv, Trw, TrU, TrV, TrW, Tri, Trj, Trk, TrI, TrJ, TrK": "Each of these values are the same as their values if you strip the 'T' prefix. However these values will only be updated once when the projectile enters the world.",
	"variable Bx, By, Bz, Br": "{Bx}, {By} and {Bz} describe the size of the caster where each describes the width, height and depth. {Br} is the length of the diagonal from the center of the caster to corner of the rectangle with width and length {Bx}x{Bz}.",
