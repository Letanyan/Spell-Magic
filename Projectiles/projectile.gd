	...
	match spell.element:
		...
		Spell.Element.ROCK:
			if body != get_node("body"):
				if is_world or is_world_object:
					...
					CharacterCollision.handle(body, self)
					dmg = body.vitals.handle_damage(Spell.Element.ROCK, spell.damage(body.vitals), spell.elemental_application)
					lose_control(self, body)
		Spell.Element.WATER:
			if is_world or is_rock or is_world_object or is_electric or is_ice:
				...
				CharacterCollision.handle(body, self)
				dmg = body.vitals.handle_damage(Spell.Element.WATER, spell.damage(body.vitals), spell.elemental_application)
				expire_now(self, body)
		Spell.Element.AIR:
			if is_world or is_world_object:
				nothing(self, body)
				...
				CharacterCollision.handle(body, self)
				dmg = body.vitals.handle_damage(Spell.Element.AIR, spell.damage(body.vitals), spell.elemental_application)
				nothing(self, body)
			elif is_rock:
		...
		Spell.Element.ICE:
			if is_world or is_rock or is_world_object or is_fire:
				...
				CharacterCollision.handle(body, self)
				dmg = body.vitals.handle_damage(Spell.Element.ICE, spell.damage(body.vitals), spell.elemental_application)
				nothing(self, body)
		Spell.Element.ELECTRIC:
