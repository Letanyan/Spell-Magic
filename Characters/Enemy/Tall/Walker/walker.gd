...
func _ready() -> void:
	super._ready()
	spell_was_cast.connect(spell_was_cast_impl)
...
func setup(seedling: int, biome: World.Biome) -> void:
	...
	ice_wall_duration = 10.0
	ice_wall.configure({"size":"vec(0.249, 0.5, 0.01)"}, Spell.Element.ICE, ice_wall_duration, 0, 3, 4, 0, 0, 0)
...
func update_behaviour() -> void:
	...
	if is_idle:
		...
		if ice_wall_timer == 0 or ice_wall_timer > ice_wall_duration:
