...
func handle_damage(kind: Spell.Element, power: float, gauge: float) -> Dictionary:
	match kind:
		...
		Spell.Element.AIR:
			burning.apply(burning.value * (1 + gauge))
			wetness.apply(wetness.value * -(1 + gauge))
			freeze.apply(freeze.value * -(1 + gauge) * 0.5)
...
static func apply_damage(world: Node3D, body: Node3D, amount: float, element: Spell.Element, show_label: bool, show_exp: bool, locations: Array[Vector3], r: float = 0, v: Vector3 = Vector3.ZERO, vitals: Vitals = null) -> void:
	...
	if show_label:
		...
		lbl.set_rise_position_change(Rand.point_in_sphere(1))
