...
func setup(seedling: int, biome: World.Biome) -> void:
	...
	var cube_path := Pathway.new() \
		.move_to(start_point) \
		.line_to(Rand.v3_abs(x_size, y_size, z_size), fit(6,3), Easing.in_out_sine) \
		.line_to(Rand.v3_abs(x_size, y_size, z_size), fit(4,2), Easing.in_out_sine) \
		.line_to(Rand.v3_abs(x_size, y_size, z_size), fit(2,1), Easing.in_out_sine) \
		.line_to(Rand.v3_abs(x_size, y_size, z_size), fit(4,2), Easing.in_out_sine) \
